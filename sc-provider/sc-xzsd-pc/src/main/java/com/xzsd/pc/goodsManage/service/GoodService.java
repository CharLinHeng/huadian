...
//        String goodCode = RandomCode.random_GoodCode();
        good.setGood_code(goodCode);
//        good.setGood_through_num(Integer.parseInt(goodCode));
        System.out.println("***********************************************************8");
        System.out.println(goodCode);
        int result = goodDao.addGood(good);
//
//        long goodCodeValue = Long.valueOf(goodCode);
//        String explain = "";
//        if(goodCodeValue <5001){
//            explain = "这个数据存到 库1表1了!";
//        }else if(goodCodeValue <10001){
//            explain = "这个数据存到 库1表2了!";
//        }else if(goodCodeValue <15001){
//            explain = "这个数据存到 库2表1了!";
//        }else{
//            explain = "这个数据存到 库2表2了!";
//        }
        if(result>0){
...
//    }
    ...
    public ResponceData queryGoodList(Good good){
        ...
        //查询redis中是否有此条件数据
        if(null ==redisTemplate.opsForValue().get(keyName)){
//            System.out.println("----------");
            ...
            if(pageData.getTotal() > 0 ){
                ...
                //插入到Redis
                String dataSet  = JSON.toJSONString(pageData);

                redisTemplate.opsForValue().set(keyName,dataSet,300, TimeUnit.SECONDS);
            ...
            //否则就直接查询Redis库中的数据。
            String goodsListJson = redisTemplate.opsForValue().get(keyName);
            if(goodsListJson.length()>0){

                //String 转json
                JSONObject jsonObject = JSONObject.parseObject(goodsListJson);

                //json 转对象


                responceData = new ResponceData(ResponceDataState.values()[0].getCode(),"查询成功,从Redis中查的!",jsonObject);
            }
        }
